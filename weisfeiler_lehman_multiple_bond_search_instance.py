...
# to create my handmade feature vectors
...
class WeisfeilerLehmanWithMultipleBonds(Kernel):
...
#         print("init")
        super(WeisfeilerLehmanWithMultipleBonds, self).__init__(
    ...
    def initialize(self):
        ...
        super(WeisfeilerLehmanWithMultipleBonds, self).initialize()
...
#                 print("This is L[nx]", L[nx])
        ...
        # Initialise Custom change for ordered feature vectors. 
        self._feat_vectors = dict()
        
        self._patterns = dict()
        
        feature_vetors = [[] for _ in range(nx)]
        for j in range(nx):
            labs = dict()
            for k in L[j].keys():
                labs[k] = WL_labels_inverse[L[j][k]]
            feature_vector = np.zeros(label_count)
            for node, value in labs.items():
                feature_vector[value] += 1
            feature_vetors[j] = feature_vector
#         print("feat_vectors", feature_vetors)
        
        self._feat_vectors[0] = np.asarray(feature_vetors)
        self._patterns[0] = L
        ...
        def generate_graphs(label_count, WL_labels_inverse):
            ...
            for j in range(nx):
                ...
                # add new labels
                ##############################################
#                 print("new graph is", (Gs_ed[j], new_labels) + extras[j])
            ...
            for i in range(1, self._n_iter):
                ...
                for j in range(nx):
                    ...
                    for v in Gs_ed[j].keys():
                        ...
                        #                         print("interest:", Gs_ed[j][v].keys())
                        #                         print("interest:", Gs_ed[j][v].values())

                        neighbours = []
                        for n in Gs_ed[j][v]:
                            for n_of_edges in range(int(Gs_ed[j][v][n])):
                                neighbours.append(L[j][n])
                        credential = str(L[j][v]) + "," + \
                                     str(sorted(neighbours))
                        #                         credential = str(L[j][v]) + "," + \
                        #                             str(sorted([L[j][n] for n in Gs_ed[j][v].keys()]))
                        
                        # Test if credential is what we are looking for: 
#                         if credential == '21,[65, 70]':
#                             print("v is ", v)
#                             print(Gs_ed[j])
...
#                         if i ==1:
#                             print(Gs_ed[j][v].keys())
#                             print(Gs_ed[j][v])
#                             print(credential)

                # assign a number to each label at this level
                ...
                for j in range(nx):
                    ...
                    L[j] = new_labels
                    # relabel
#                     print("new graph rellabeled is", new_labels)
#                     print("new graph rellabeled values is", new_labels.values())
#                     set_of_values = new_labels.values()
#                     print("new graph rellabeled values is", np.min(set_of_values))
                    key_max = max(WL_labels_inverse.keys(), key=(lambda k: WL_labels_inverse[k]))
                    key_min = min(WL_labels_inverse.keys(), key=(lambda k: WL_labels_inverse[k]))

#                     print('Maximum Value: ',WL_labels_inverse[key_max])
#                     print('Minimum Value: ',WL_labels_inverse[key_min])
                    feature_vector = np.zeros(WL_labels_inverse[key_max] - WL_labels_inverse[key_min] + 1)
                    for node, value in new_labels.items():
                        feature_vector[value - WL_labels_inverse[key_min]] += 1
                    # This feature vector has the same feature ordering as the WL_labels_inverse. 
                    feature_vetors[j] = feature_vector
                    new_graphs.append((Gs_ed[j], new_labels) + extras[j])
                self._inv_labels[i] = WL_labels_inverse
                
                self._feat_vectors[i] = np.asarray(feature_vetors)
                self._patterns[i] = L_temp
#                 print(new_graphs)
                yield new_graphs
...
#                         L_temp[j][v] = credential
                ...
                # Recalculate labels
                ## IF I WANT AN ORDERED FEATURE VECTOR HERE I NEED TO CHANGE THE BELOW. THE CURRENT FEATURE VECTOR ONLY WORKS FOR FIT_TRANFORM ALL TOGETHER. 
                new_graphs = list()
                for j in range(nx):
                    new_labels = dict()
        ...
        # Adding L_temp to the outputs
#         self._patterns[2] = L_temp
